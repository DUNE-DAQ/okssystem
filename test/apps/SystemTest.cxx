 ...
 *  Test
 *
 *  JCF, Oct-17-2022: SystemTest as written by Matthias Wiesmann has
...
void test_write_chmod(const System::File &file) {
  TLOG_DEBUG( 1) << "Testing System::File::output " << file.c_full_name() ;
    std::ostream* stream= file.output();
    (*stream) << "#!/bin/sh\necho $1\nexit $1\n";
    delete(stream); 
    mode_t permission = 0700;
    TLOG_DEBUG( 1) << "Testing System::File::permissions " << file.c_full_name();
    TLOG_DEBUG( 1) << "Setting permissions: " << System::File::pretty_permissions(permission).c_str() << " " << permission;
...
int test_exec(const System::Executable &executable, int status ) {
  TLOG_DEBUG( 1) << "Testing System::Executable::start \"" << executable.c_full_name() << "\" parameter: " << status; 
