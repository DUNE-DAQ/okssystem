...
int
OksSystem::Descriptor::flags(bool read_mode, bool write_mode)
{
...
void
OksSystem::Descriptor::open(const File* file, int i_flags, mode_t perm)
{
  ERS_ASSERT(file)
  ...
  if (m_fd < 0)
    throw OksSystem::OpenFileIssue(ERS_HERE, errno, m_name.c_str());
...
void
OksSystem::Descriptor::close_safe() throw()
{
  const int status = ::close(m_fd);
  if (status < 0) {
...
int
OksSystem::Descriptor::read(void* buffer, size_t number) const
{
  ssize_t status = ::read(m_fd, buffer, number);
  if (status < 0)
    throw OksSystem::ReadIssue(ERS_HERE, errno, m_name.c_str());
  return status;
} // read

int
OksSystem::Descriptor::write(const void* buffer, size_t number) const
{
  ssize_t status = ::write(m_fd, buffer, number);
  if (status < 0)
    throw OksSystem::WriteIssue(ERS_HERE, errno, m_name.c_str());
  return status;
} // write
