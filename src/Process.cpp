...
const int OksSystem::Process::TERMINATION_WAIT =
  100000; /**< Wait time before deciding a termination signal did not work (in nanoseconds) */

const char* const OksSystem::Process::SYS_EXITS_NAMES[] = {
  "command line usage error", "data format error",
  "cannot open input",        "addressee unknown",
  "host name unknown",        "service unavailable",
  "internal software error",  "okssystem error",
  "critical OS file missing", "can't create (user) output file",
  "input/output error",       "temp failure; user is invited to retry",
  "remote error in protocol", "permission denied ",
  "configuration error"
};
const char* const OksSystem::Process::TEST_EXITS_NAMES[] = { "undefined test",
                                                             "test failed",
                                                             "test unresolved",
                                                             "test untested",
                                                             "unsupported test" };
...
const char*
OksSystem::Process::exit_text(int return_value)
{
  if (0 == return_value)
    return OK_EXIT_NAME;
  if ((return_value >= EX__BASE) && (return_value <= EX__MAX))
    return SYS_EXITS_NAMES[return_value - EX__BASE];
  if ((return_value >= TEST_BASE_VALUE) && (return_value <= TEST_MAX_VALUE))
    return TEST_EXITS_NAMES[return_value - TEST_BASE_VALUE];
  return 0;
} // return_text
...
std::string
OksSystem::Process::exit_pretty(int return_value)
{
...
const OksSystem::Process*
OksSystem::Process::instance() throw()
{
  if (0 == s_instance) {
...
void
OksSystem::Process::set_name(const std::string& name) throw()
{
  (void)instance();
...
int
OksSystem::Process::join(bool throw_non_zero) const
{
  ...
  if (WIFEXITED(status)) {
    ...
    if ((throw_non_zero == false) || (exit_status == 0))
      return exit_status;
    throw OksSystem::TerminationIssue(ERS_HERE, errno, exit_status);
  } // exit status
  if (WIFSIGNALED(status))
    throw OksSystem::SignalIssue(ERS_HERE, errno, WTERMSIG(status));
  if (WIFSTOPPED(status))
    throw OksSystem::SignalIssue(ERS_HERE, errno, WSTOPSIG(status));
...
void
OksSystem::Process::signal(int signal_number) const
{
...
bool
OksSystem::Process::exists() const
{
...
bool
OksSystem::Process::equals(const Process& other) const throw()
{
...
void
OksSystem::Process::terminate() const
{
  /*
...
std::string
OksSystem::Process::to_string() const throw()
{
  std::ostringstream stream;
...
pid_t
OksSystem::Process::process_id() const throw()
{
  return m_process_id;
}
...
std::ostream&
operator<<(std::ostream& out, const OksSystem::Process& proc)
{
  out << proc.to_string();
  return out;
} // operator<<
...
bool
operator==(const OksSystem::Process& a, const OksSystem::Process& b) throw()
{
  return a.equals(b);
} // operator ==
