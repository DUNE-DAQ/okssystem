...
std::string
OksSystem::Executable::okssystem(const std::string& command)
{
...
void
OksSystem::Executable::exec(char** argv) const
{
 ...
 */
void
OksSystem::Executable::exec(char** const argv, char** const env) const
{
  ERS_PRECONDITION(argv);
  ERS_PRECONDITION(argv[0]);
...
void
OksSystem::Executable::exec(const param_collection& params) const
{
...
void
OksSystem::Executable::exec(const param_collection& params, const env_collection& envs) const
{
...
OksSystem::Process
OksSystem::Executable::start(const param_collection& params) const
{
...
/** This method starts the executable in another process using \c fork. The parent will not wait on the child
 * termination.
 * \param params the parameters to use when launching the executable
 * \return A process object representing the started process
 */
...
OksSystem::Process
OksSystem::Executable::start_and_forget(const param_collection& params) const
{
...
void
OksSystem::Executable::copy_fd(int fd, std::ostream& target)
{
  while (true) {
    char buffer[256];
...
std::string
OksSystem::Executable::pipe_in(const param_collection& params) const
{
  ...
  if (input_pipe_status < 0)
    throw OksSystem::OksSystemCallIssue(ERS_HERE, errno, "pipe", "");
  const int data_pipe_status = pipe(data_pipe);
  if (data_pipe_status < 0)
    throw OksSystem::OksSystemCallIssue(ERS_HERE, errno, "pipe", "");
  const int error_pipe_status = pipe(error_pipe);
  if (error_pipe_status < 0)
    throw OksSystem::OksSystemCallIssue(ERS_HERE, errno, "pipe", "");
...
std::string
OksSystem::Executable::pipe_in(const param_collection& params, const env_collection& envs) const
{
...
OksSystem::Process
OksSystem::Executable::pipe_out(const param_collection& params,
                                ...
                                mode_t perm) const
{
  ...
  OksSystem::Descriptor in(
    &input_file, OksSystem::Descriptor::flags(true, false), perm); // May throw OksSystem::OpenFileIssue
...
OksSystem::Process
OksSystem::Executable::start(const param_collection& params, const env_collection& envs) const
{
...
std::string
OksSystem::Executable::to_string(const param_collection& params) const
{
