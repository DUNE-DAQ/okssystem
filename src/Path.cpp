...
void
OksSystem::Path::add(const OksSystem::File& dir)
{
...
void
OksSystem::Path::parse_path_list(const std::string& path_list)
{
...
void
OksSystem::Path::write_to(std::ostream& stream) const
{
  bool first = true;
...
std::string
OksSystem::Path::to_string() const
{
...
OksSystem::File
OksSystem::Path::which(const std::string& name) const
{
  for (File::file_list_t::const_iterator pos = m_directories.begin(); pos != m_directories.end(); pos++) {
...
std::ostream&
operator<<(std::ostream& stream, const OksSystem::Path& path)
{
