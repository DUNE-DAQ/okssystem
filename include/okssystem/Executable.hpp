...
protected:
  ...
  void exec(char** const argv, char** const env)
    const; /**< \brief does the actual \c exec setting the right environment for the child process*/
  ...
  static void copy_fd(int fd,
                      std::ostream& target); /**< \brief copies the content of a file descriptor into a STL stream */
public:
  ...
  Process pipe_out(const param_collection& params,
  ...
  Process start_and_forget(const param_collection& params)
    const; /**< \brief start the executable in another process; do not wait for termination of child */
  Process start(const param_collection& params,
                const env_collection& envs) const; /**< \brief start the executable in another process */

  std::string to_string(const param_collection& params)
    const; /**< \brief converts executable name and a parameter sequence into a string */
